...
- **Kill-switch — currently DISABLED (experimental):** a WFP dynamic-session kill-switch is prototyped in `windows/killswitch/` (Go, `github.com/tailscale/wf`), but its `permit-tun` filter didn't correctly match tunneled traffic and blocked new connections (DNS/YouTube/games). It is **not wired into the tray** and `asena-on` never enables it, pending a proper Windows test loop. The Go source is kept for a future, Windows-verified version.
...
- **Kill-switch — şu an DEVRE DIŞI (deneysel):** `windows/killswitch/` altında WFP dynamic-session tabanlı bir kill-switch prototipi var (Go, `github.com/tailscale/wf`), ama `permit-tun` filtresi tünel trafiğini doğru eşleştiremeyip yeni bağlantıları (DNS/YouTube/oyun) blokluyordu. Tray'e **bağlı değil** ve `asena-on` onu asla açmıyor — Windows'ta düzgün bir test döngüsü kurulana kadar. Go kaynağı ileriki (Windows'ta doğrulanmış) sürüm için saklanıyor.
