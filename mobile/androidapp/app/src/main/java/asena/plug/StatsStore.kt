...
object StatsStore {
    ...
    private const val DL_STREAMS = 4                  // paralel indirme akışı — tek akış tünel/yüksek-gecikmede
                                                      // BDP-limitli kalıp gerçek hızın altını gösteriyordu (upload etkilenmiyor)
    private const val WARMUP_NS = 900_000_000L        // ~0.9 sn: TCP slow-start'ı ölçüme dahil ETME
    ...
    private fun testDownload() {
        // PARALEL: DL_STREAMS eşzamanlı akış -> linki doyurur (tek akış BDP-limitli kalıyordu).
        // Her akış EOF'ta (50MB) yeniden bağlanır -> pencere boyunca sürekli veri. Sayaç paylaşımlı.
        val total = AtomicLong(0)
        val stop = AtomicBoolean(false)
        val threads = (0 until DL_STREAMS).map {
            Thread {
                while (!stop.get()) {
                    try {
                        ...
                        if (c.responseCode != 200) { runCatching { c.errorStream?.close() }; break }
                        c.inputStream.use { s ->
                            val buf = ByteArray(65536)
                            while (!stop.get()) {
                                val n = s.read(buf); if (n < 0) break   // EOF -> dış döngü yeniden bağlanır
        ...
        // warm-up: slow-start geçsin; sonra DELTA (steady-state) ölç -> gerçek hız.
        try { Thread.sleep(WARMUP_NS / 1_000_000) } catch (_: Exception) {}
        val base = total.get()
        ...
        while (true) {
            ...
            if (now - lastEmit >= EMIT_NS) {
                downloadMbps.value = mbps(total.get() - base, now - t0)   // CANLI
                lastEmit = now
            }
            if (now - t0 >= WINDOW_NS) break
        }
        downloadMbps.value = mbps(total.get() - base, System.nanoTime() - t0)  // sonuç
        stop.set(true)
        threads.forEach { runCatching { it.join(600) } }
    }
